...
Do not create recurring automations, scheduled jobs, tickets, deployment actions, tags, or ticket-state mutations from this skill unless the user explicitly requests that separate action.
...
3. Add or reuse a compact ticket comment with marker:
...
4. Treat failure to write the local result or ticket provider marker as an audit evidence gap, not a PROD deployment failure, after PROD success was already recorded.
...
- local result path and ticket provider marker status when the scope is `post-prod-ticket-release`.
