...
16. Create an OpenSpec proposal using the `dev-flow-propose-change` skill (`/opsx:propose`) with a change name matching the branch name as closely as OpenSpec allows, unless OpenSpec is explicitly skipped.
...
Report the selected ticket, branch, OpenSpec change or explicit no-OpenSpec rationale, ticket lock path, telemetry initialization, validation performed, Plane comment marker, and handoff to `dev-flow-implement-ticket`.
