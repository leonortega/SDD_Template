...
This repo-local copy preserves the upstream test-analysis extension lookup tables and adds the delivery context required by this repository.

## Shared Context

Source: https://github.com/dotnet/skills/tree/main/plugins/dotnet-test/skills/test-analysis-extensions

Before using these extension files for ticketed delivery, read `.codex/skills/_shared/delivery-contract.md` and `docs/context-management.md`. Apply the active ticket scope, repository validation gates, and handoff expectations before using extension data to change tests or report test quality findings.

## Workflow

Detect the project language and test framework, read the matching extension file, and pass those framework-specific assertion or test markers to the analysis skill that invoked this one.

## Output

Return the extension files selected, the detected framework evidence, any validation implications, and ticket or handoff notes needed by the invoking analysis skill.

## Failure Rules

Stop and report a blocker when no extension matches the project language, framework detection is ambiguous, or using the fallback extension would make the analysis unreliable for the active ticket.
