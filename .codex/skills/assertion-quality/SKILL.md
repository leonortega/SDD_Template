...
This repo-local copy preserves the upstream assertion-quality guidance and adds the delivery context required by this repository.

## Shared Context

Source: https://github.com/dotnet/skills/tree/main/plugins/dotnet-test/skills/assertion-quality

Before using this skill for ticketed delivery, read `.codex/skills/_shared/delivery-contract.md` and `docs/context-management.md`. Apply the active ticket scope, repository test conventions, validation gates, and handoff expectations before changing tests or reporting test quality findings.

This skill depends on the repo-local `test-analysis-extensions` skill. Read the relevant extension file, such as `.codex/skills/test-analysis-extensions/extensions/dotnet.md`, before classifying xUnit, MSTest, NUnit, or TUnit assertions.

## Output

Report assertion-quality metrics, actionable findings, files reviewed, validation commands run, and any ticket or handoff notes needed for implementation or QA.

## Failure Rules

Stop and report a blocker when the test framework cannot be identified, the required extension guidance is missing, test files are outside the active ticket scope, or validation cannot be run after test changes.
