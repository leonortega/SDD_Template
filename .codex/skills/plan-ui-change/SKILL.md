...
This repo-local copy preserves the upstream Blazor UI planning guidance and adds the delivery context required by this repository.

## Shared Context

Source: https://github.com/dotnet/skills/tree/main/plugins/dotnet-blazor/skills/plan-ui-change

Before using this skill for ticketed delivery, read `.codex/skills/_shared/delivery-contract.md` and `docs/context-management.md`. Apply the active ticket scope, Blazor project conventions, validation gates, and handoff expectations before changing UI components, tests, docs, or QA evidence.

## Workflow

Follow the upstream component planning workflow above. After implementation, run the repository validation appropriate for the touched Blazor files and include the UI decomposition, test coverage, and handoff notes in the ticket workflow.

## Output

Return the component plan, files changed, validation commands run, and any ticket or handoff notes required for review or QA.

## Failure Rules

Stop and report a blocker when the requested UI change conflicts with the active ticket, requires missing design or acceptance criteria, cannot be validated locally, or would introduce broad unrelated refactoring.
