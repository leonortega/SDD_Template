...
Repo-local adaptation: this repository does not use the upstream `CONTEXT.md` and ADR output convention by default. Preserve the domain-modeling behavior, but capture durable output in the existing SDD surfaces: OpenProject generated blocks for product or ticket clarity, OpenSpec artifacts for planned behavior and design, `docs/` for durable repository or process knowledge, and `knowledge/` for reusable non-authoritative lessons. Create `CONTEXT.md` or ADR files only after a separate explicit repo change adopts that model.
