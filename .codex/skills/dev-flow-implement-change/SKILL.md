...
Use this skill to take an active OpenSpec change from implementation through Gitea PR handoff. Compose the existing `dev-flow-apply-change` workflow with local verification, Git, Gitea, the `dev-flow-pr-review-agent` skill, and Plane API updates.
