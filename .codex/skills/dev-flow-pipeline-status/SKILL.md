...
Before reporting, follow `.codex/skills/_shared/skill-startup.md`, which reads `.codex/project-profile.json`, `.codex/skills/_shared/delivery-contract.md`, and `docs/conventions/context-management.md`, with `docs/architecture/system.md` as the stage-specific doc. Load only adapters needed for read-only status. This remains a read-only skill; do not update knowledge unless the user explicitly asks for a workflow-memory correction.
