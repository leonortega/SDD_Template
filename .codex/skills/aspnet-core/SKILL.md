...
Before using this repo-local copy for ticketed delivery, read `.codex/skills/_shared/delivery-contract.md` and `docs/context-management.md`. Apply the current ticket context, repository validation gates, and handoff rules before changing code, tests, docs, QA evidence, deployment behavior, or local workflow files.

## Output

When this skill influences a change, report the ASP.NET Core guidance used, files touched, validation commands run, and any ticket or handoff notes needed by the repository workflow.

## Failure Rules

Stop and report a blocker when the source guidance conflicts with the active ticket, the repository delivery contract, or current project conventions. Do not introduce preview framework APIs, secret-bearing configuration, deployment changes, or broad architecture rewrites without explicit user intent and matching validation.
