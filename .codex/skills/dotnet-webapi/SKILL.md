...
This repo-local copy preserves the upstream ASP.NET Core Web API guidance and adds the delivery context required by this repository.

## Shared Context

Source: https://github.com/dotnet/skills/tree/main/plugins/dotnet-aspnet/skills/dotnet-webapi

Before using this skill for ticketed delivery, read `.codex/skills/_shared/delivery-contract.md` and `docs/context-management.md`. Apply the active ticket scope, API style already present in the project, validation gates, and handoff expectations before changing endpoints, tests, OpenAPI metadata, or error handling.

## Output

Report the API style chosen, endpoints or DTOs changed, validation commands run, and any ticket or handoff notes needed for review, QA, or deployment.

## Failure Rules

Stop and report a blocker when API behavior is ambiguous, the change would mix incompatible endpoint styles, required validation cannot be run, or the requested work conflicts with the active ticket or repository delivery contract.
