...
Use the new modular CLI: `python -m tools.sdd_cli environment-lab` for environment setup and `python -m tools.sdd_cli dev-flow` for quality-gate audits.
...
1. Run `python -m tools.sdd_cli dev-flow audit-skill-contracts`.
2. If templates are missing, ask before running `python -m tools.sdd_cli environment-lab init-quality-templates`.
...
5. Use `python -m tools.sdd_cli environment-lab set-quality-config` when the user wants a different coverage threshold; never write scanner, Gitea, Nexus, Azure, or OpenProject secrets there.
...
7. Run `python -m tools.sdd_cli environment-lab build-gitea-images` before any PR validation, deployment, or QA E2E workflow. .NET SDK tooling, Gitleaks, Trivy, Azure CLI, jq, zip, Node, and Playwright runtime dependencies must come from pinned local CI images. Host installs are not allowed when a pinned Docker image is available.
8. Run `python -m tools.sdd_cli environment-lab validate-gitea-runner` when Docker is available to catch missing local job images, missing shell tools, JavaScript action/node mismatches, and local Gitea checkout networking before a PR depends on CI.
