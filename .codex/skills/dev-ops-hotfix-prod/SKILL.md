...
Before starting, follow `.codex/skills/_shared/skill-startup.md`, which reads `.codex/project-profile.json`,
...
2. Read `.codex/delivery-context.local.json` when present. If it points to an unrelated active feature ticket, stop and
...
`.codex/skills/_shared/pipeline-tdd-cycle.md`. Key hotfix-specific details:
   ...
   implementation:
     ...
     `.codex/skills/_shared/test-requirements.md` (driven by `stack.testFrameworks`) plus the **coverage gate** with the
