...
# Pre-Flight: Skills & MCP Gate

**This is a mandatory pre-flight gate (authority level 5).** Before any mutation — before editing files, making API calls, committing, pushing, or changing ticket state — every agent **must** check which skills and MCPs apply to the current task and activate them.
...
## 2. Check MCPs For The Current Domain
...
| ------------------------------- | ----------------------- | -------------------------------------- |
| Documentation, skills, markdown | `monorepo-docs-search`  | `search_documentation`                 |
...
- Verify the MCP is configured in `.vscode/mcp.json` or `.cline/mcp_settings.json`.
...
- For **MCPs**: route all searches through them (code → codebase-memory-mcp, docs → monorepo-docs-search, services → service MCPs)
...
- Which MCPs were **used** for this step (e.g., "MCPs used: codebase-memory-mcp for code search")
...
- MCPs used: monorepo-docs-search for skill lookup, codebase-memory-mcp for code search
```
