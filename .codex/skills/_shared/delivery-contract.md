...
For the k3d local lane, the artifact identity also includes the digest-pinned container image set:
...
`deployable-apps.json` is the packaged copy of `infra/deployment/apps.json` sorted for deployment. `commit.sha` must exactly match the artifact commit. Every `{artifactName}.sha256` listed by the topology must verify before ZIP deployment. For k3d deployment, every image reference in `container-images.json` must be pinned by `@sha256:` digest, and DEV, QA, PROD, and rollback must deploy those exact digest references.

k3d observability evidence uses direct site/API `/health` summaries. The local lane publishes `monitoring-summary*.json` and `qa-observability.json` under `app/{commitSha}/`; QA evidence bundles may include those summaries, but must not include kubeconfig, registry credentials, connection strings, tokens, or pod logs. Grafana Infinity dashboards and alerts provide operator visibility over the same health URLs but are not the deployment evidence source.
