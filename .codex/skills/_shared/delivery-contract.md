...
Rancher Desktop observability evidence must use sanitized logs only. The local lane may publish `monitoring-summary*.json`, `qa-observability.json`, and sanitized pod-log files under `app/{commitSha}/`; QA evidence bundles may include those summaries, but must not include kubeconfig, registry credentials, connection strings, tokens, or unsanitized pod logs.
