...
- When searching the repository for source code (non-`.md`/`.mdx` files), always use `codebase-memory-mcp` as the first source. Use `search_graph` (BM25 ranking + structural boosting) for natural-language or name-based discovery, `get_architecture`/`trace_path`/`get_code_snippet`/`query_graph` for structural analysis, and `search_code` (grep + graph-enriched dedup) for raw regex/pattern matching. See `.codex/mcp-instructions.md` for the full routing contract. Do not use raw grep as the first approach for code discovery.
