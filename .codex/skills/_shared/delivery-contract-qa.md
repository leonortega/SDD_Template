...
**How it works:** The `playwright.config.ts` (generated by the AI-driven `dev-flow-scaffold-project` skill after the tech stack is defined — only for JS/TS web stacks; never a fixed template list) reads the `BASE_URL` environment variable at the top of the config:
