...
description: "Persistent browser and Electron interaction through `js_repl` for fast iterative UI debugging. Use in this repository for local UI iteration, visual QA, viewport-fit checks, and deployed browser failure diagnosis; do not use as the official E2E QA gate."
---
...
Use this skill for fast iterative browser or Electron debugging, visual QA, viewport-fit checks, and UI failure diagnosis with a persistent Playwright session.
...
Use the core workflow below. For this repository, first decide whether the Browser plugin already covers the check. Use this skill when persistent Playwright handles make iteration, visual QA, or failure diagnosis materially faster.
...
Report the target URL, session mode, viewport coverage, functional checks, visual checks, screenshots or traces captured, failure classification, and any residual risk. For ticket delivery, include whether evidence stayed under ignored `artifacts/qa/**` and whether committed Playwright coverage remains the acceptance source.
...
- Stop before treating screenshots, traces, visual inspection, page loads, or console health as a replacement for committed executable assertions.
