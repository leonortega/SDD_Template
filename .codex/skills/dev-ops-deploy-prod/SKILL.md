...
ticket key format configured in `.codex/project-profile.json` at `workflow.ticketKeyPattern`, and the change must touch
...
6. Verify every included ticket's E2E QA comment includes pass result, PR URL, QA URL, Nexus artifact URL, QA evidence
   ...
   **⚠️ E2E QA gate:** If any included ticket is missing a passing E2E QA comment (`IA generated E2E QA: {ticketKey}`
   ...
   `.codex/skills/_shared/delivery-contract-qa.md`
...
- selected deployment provider: a push/merge to `main` deploys PROD only when the changed files include
  ...
  `.codex/project-profile.json`. It resolves the artifact from `app/qa-approved/latest.json`, requires the pointer
...
Maintenance-only
changes such as `.codex/**` or workflow-only edits must not deploy PROD. PROD must download the selected provider
artifact
...
1. Run `python -m tools.sdd_cli agent-eval run` (fails loudly with a clear error if
   ...
   .codex/agent-evals/promptfooconfig.yaml --no-cache`)
2. Read the results:
   - **All tests pass**: report "no routing regressions detected"
   - **Any test fails**: flag the failures for the retrospective audit

Persist the eval results to `.codex/agent-evals/results.local.json` with mode `post-prod-eval`, scope set to the final
...
`.codex/agent-evals/results.local.json` with mode `eval-driven-improvement` and
sub-mode `probe`.
...
- **Only eval files**: Never auto-apply changes outside `.codex/agent-evals/routing_provider.py` or
  `.codex/agent-evals/promptfooconfig.yaml`. If a fix would require changing a delivery skill,
