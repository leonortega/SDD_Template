...
2. When Rancher Desktop is the selected deployment provider and the user explicitly asked for `config infra`, full setup, or Rancher Desktop local lab setup, run `EnsureRancherKubernetes` before `Audit`. This is the only configure step that may auto-enable Rancher Desktop Kubernetes; plain `Audit` remains read-only.
...
25. If the user is vague, default to full guided setup in this order:
   InitProjectProfile -> EnsureRancherKubernetes when Rancher Desktop is selected -> Audit -> Plane -> Gitea PR automation -> Gitea Actions runner -> Quality gates and CI -> Nexus artifacts and Rancher Desktop deployment promotion -> Monitoring dashboards -> final Audit.
