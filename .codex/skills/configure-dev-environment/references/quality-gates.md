...
Run `BuildGiteaActionsImages` after changing image Dockerfiles, tool versions, workflow image tags, or Rancher Desktop local-lab workflow requirements. Then run `ValidateGiteaActionsRunner`; it validates Docker availability, local image presence, required tools inside each image, including Docker CLI and `kubectl` in the .NET CI image, and that the PR validation job image can reach the repository origin through `host.docker.internal`.
...
E2E QA is an acceptance-evidence gate. Implementation owns committed automated coverage for every acceptance criterion, including Playwright E2E tests when browser-level proof is required. After QA deployment, `quality-test-e2e` uses the temporary `qa/{ticketKey}` branch to run existing committed tests only; it must not create, repair, stage, or commit tests when current coverage cannot prove acceptance. Gitea Actions `e2e-qa` is the normal QA execution path. Local E2E QA is forbidden unless a deployment-related blocker prevents that remote path from running or completing and 2 deploy-fix attempts have failed. Product E2E test failures remain QA failures and are not a local-fallback trigger. Any local fallback must use `npm run test:docker` in `tests/SDDTemplate.E2ETests`, target deployed QA URLs through `E2E_SITE_URL` and `E2E_API_URL`, never target localhost, and record the blocker, both failed fix attempts, local command, tested URLs, and evidence. During QA, one-off exploratory probes stay under ignored `artifacts/qa/**` and are evidence, not committed regression coverage. For deployed browser E2E failures, Playwright MCP or the configured Browser/Playwright tool is the first diagnostic source before source-code changes; classify the failure as product defect, committed-test defect, deployment/environment issue, or workflow gate gap. App code must remain product-only and must not receive E2E-only JavaScript helpers, hidden hooks, test ids, bypasses, timing shims, or Playwright-specific behavior. Missing committed coverage or committed-test defects route back to implementation/review and keep the work package in QA. `quality-test-e2e` may move a OpenProject work package to Done only when the deployed QA artifact receives a full `PASS`: ticket acceptance criteria are mapped to executable assertions from existing committed tests, relevant user workflow/API/backend/state/validation/error/environment/evidence-integrity scenarios are covered, and screenshots or smoke checks support rather than replace assertions. `PASS WITH GAPS` and `FAIL` must remain in QA.
