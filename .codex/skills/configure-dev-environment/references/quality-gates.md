...
DEV and QA deploy only from `dev` when application/test/package source changed. PROD deploys only from `main` for the same ticket-gated application changes. Manual workflow dispatch remains available for explicit promotion.
...
The package/deploy workflow should build and publish from ticket-gated application changes on `dev`. DEV and QA deployments must download the same Nexus ZIP for the same commit SHA and pass page plus `/health` checks. PROD must reuse the QA-passed artifact commit after `main` is updated or explicit dispatch is requested, using `artifact_commit_sha`, `release_version`, and `source_rc_version` for dispatch promotions; PROD must not rebuild and must pass page plus `/health` checks before success is recorded. Release automation should update `app/{commitSha}/release.json` instead of renaming `app.zip`.
