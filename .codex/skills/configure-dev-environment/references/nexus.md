...
The Rancher Desktop workflow uploads image metadata to raw Nexus:
...
- For rollback, redeploy the previous known-good provider artifact set: Azure `app/{commitSha}/deployable-apps.json` ZIP topology with checksums, or Rancher Desktop `app/{commitSha}/container-images.json` digest set. Verify `/health`, update `release.json`, and comment OpenProject with rollback lineage.
