...
Use this adapter only when `.template/project-profile.json` selects `providers.repository.id` or `providers.review.id =
"gitea"`.

## Runtime Configuration

- Read non-secret branch and workflow policy from `.template/project-profile.json`.
- Read local endpoint, token, owner, repository, reviewers, and labels from `.template/client-tools.local.json`.
...
using the built-in admin credentials (`admin`/`admin123`). The token is written to `.template/client-tools.local.json`
