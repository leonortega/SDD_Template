...
Read `.agents/skills/_shared/delivery-contract.md` and `docs/conventions/context-management.md` before changing local
...
- Update only ignored local files for secrets: `.template/client-tools.local.json`, `.template/quality.local.json`,
...
1. **Check Nexus password** — verify it in client-tools config:
   ...
   ```bash
   python -c "import json; c=json.load(open('.template/client-tools.local.json')); print(c['nexus']['password'])"
   ```
...
1. Check `.template/client-tools.local.json` for `openProject.baseUrl`, `openProject.apiToken`, and
...
```bash
TOKEN=$(python -c "import json; print(json.load(open('.template/client-tools.local.json'))['gitea']['apiToken'])")
...
3. Ensure `.template/quality.local.json` exists (default coverage minimum: 80%).
