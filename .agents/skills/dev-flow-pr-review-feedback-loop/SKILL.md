...
Before reading or mutating review feedback, follow `.agents/skills/_shared/skill-startup.md`, which reads
...
Read `.template/delivery-context.local.json` when present and verify the resolved ticket, branch, PR, and current head SHA
...
`.template/client-tools.example.json` only for defaults and setup
guidance.
