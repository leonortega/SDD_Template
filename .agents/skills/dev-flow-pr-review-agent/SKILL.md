...
Before posting review output, follow `.agents/skills/_shared/skill-startup.md`, which reads
...
Read `.template/client-tools.local.json` first. Fall back to `.template/client-tools.example.json` for defaults only, then
...
Read `.template/delivery-context.local.json` when present and verify the PR number, branch, title/body ticket key, and head
