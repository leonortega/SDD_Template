...
4. **⚠️ MANDATORY: Write tests based on tasks before product code.** For each pending task:
   - Load the `tdd` skill via `skill('tdd')` (or read `.agents/skills/tdd/SKILL.md`) and apply its test-first cycles
   throughout.
   ...
   `.agents/skills/_shared/test-requirements.md` — unit tests (per component), integration tests (per
...
Per `.agents/mcp-instructions.md`, route service interactions through the service MCPs (gitea, openproject, grafana).
...
Before ticketed implementation, read `.agents/skills/_shared/delivery-contract.md` and
...
1. Read `.template/project-profile.local.json` → `stack` section for frontend/backend/database values (stack lives **only**
in the ignored local overlay). If it does not exist, stack is empty. Read
`.template/project-profile.json` for non-stack config (providers, workflow, gates).
2. Read `.template/tool-recommendations.local.json` → `accepted` list to find which skills are enabled.
