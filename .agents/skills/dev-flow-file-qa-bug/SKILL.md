...
Before filing or linking tickets, follow `.agents/skills/_shared/skill-startup.md`, which reads
...
Read `.template/project-profile.json` first. Read `.template/client-tools.local.json` only for selected adapter runtime values
...
failure output.
2. Read `.template/delivery-context.local.json` when present and verify the parent ticket, tested commit, artifact
...
5. Comment on the parent ticket with the bug link. See `.agents/skills/_shared/pipeline-ticket-comment.md` for the common
comment pattern. Use:
   - Marker: `IA generated QA bug: {parentTicketKey}`
...
### Phase 4 — Start Fix Branch
 ...
 `.agents/skills/_shared/pipeline-ticket-comment.md` for the common comment pattern. Use:
...
See `.agents/skills/_shared/pipeline-tdd-cycle.md` for the common TDD test-first pattern. Bug-flow-specific details:
...
- **Quality gates:** the same hard gates as the feature flow apply to the bug fix:
  ...
  `.agents/skills/_shared/test-requirements.md`, driven by `stack.testFrameworks` from
  `.template/project-profile.local.json`, then the **coverage gate** with the configurable threshold
  ...
  - **Coverage gate:** coverage must meet `coverage.minimumPercent` (default `80`) from `.template/quality.local.json`
...
deployment, bug closure, and return to the parent ticket's QA flow.
 ...
 3. Update `.template/delivery-context.local.json` to switch context to the bug ticket for deployment tracking:
...
- Severity: `advisory` (retry once, then continue)
 ...
 1. Update `.template/delivery-context.local.json` to point back to the **parent ticket**:
