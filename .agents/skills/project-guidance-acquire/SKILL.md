...
Use this skill to install skills into `.agents/skills/` from configured GitHub
sources, keeping the catalog in sync with `manifest.json`.

## Shared Context

Read `.agents/skills/_shared/delivery-contract.md` and
...
- `.template/skill-sources.json` or `.template/skill-sources.example.json` with configured GitHub source repos (for fallback)
...
This reads `.template/skill-sources.json` (falling back to `.template/skill-sources.example.json`, then hardcoded defaults),
...
Create `.template/skill-sources.json` with your own repos:
...
- **npx method**: skill is installed to `.agents/skills/<skill-name>/` via the skills.sh registry
- **GitHub fallback**: the entire skill directory is copied to `.agents/skills/<skill-name>/`
