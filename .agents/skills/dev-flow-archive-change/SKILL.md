...
4. **Capture durable learning (mandatory after archive)**
   ...
   (`.agents/skills/_shared/delivery-contract-core.md` → Durable Learning Capture Gate) using the
...
or the ticket's changed-file list; when unavailable, fall back to the change's own paths under `openspec/changes/<name>/`.
   ...
   Then:
   - If the classifier returns `UPDATE` candidates, load `.agents/skills/docs-knowledge-maintenance/SKILL.md`
...
Before ticketed archival, read `.agents/skills/_shared/delivery-contract.md` and
