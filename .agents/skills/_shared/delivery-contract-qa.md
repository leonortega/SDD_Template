...
The archive step also runs the **Durable Learning Capture Gate** (`.agents/skills/_shared/delivery-contract-core.md`):
...
- Playwright and Chrome are available in the `sdd-e2e-ci:local` container (never install locally — use:
  `docker run --rm -v "$(pwd):/workspace" -w /workspace sdd-e2e-ci:local npx playwright --version`)
...
```bash
docker run --rm -e BASE_URL="${QA_FRONTEND_URL}" -v "$(pwd):/workspace" -w /workspace sdd-e2e-ci:local npx playwright test --reporter=list,json
```
...
```bash
docker run --rm -e BASE_URL="${QA_FRONTEND_URL}" -v "$(pwd):/workspace" -w /workspace sdd-e2e-ci:local npx playwright test --reporter=list,json > e2e-qa-output.json 2>&1
...
`.agents/skills/_shared/pipeline-workflow-telemetry.md`):
