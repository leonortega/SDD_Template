...
Open `.agents/skills/manifest.json` and read the `categories` section. Identify which categories match the current task:
...
```bash
test -f ".agents/skills/<skill-path>" && echo "EXISTS" || echo "NOT_FOUND"
```
...
Read `.agents/mcp-instructions.md` for the mandatory MCP routing contract. Identify which MCP servers are relevant to the
current task:
