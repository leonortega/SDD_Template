...
1. **Run the reviewer automation** (deterministic resolve + request + verify + retry):
   ...
   The command reads `.template/client-tools.local.json` (`gitea.baseUrl/apiToken/owner/repo`), resolves the reviewer
...
- For API endpoint details: `.agents/skills/_shared/api-helpers.md` → Gitea → Request reviewers
- For provider-specific API: `.agents/skills/dev-flow-pr-review-agent/references/gitea-review-api.md` →
