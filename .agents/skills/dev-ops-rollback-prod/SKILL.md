...
Before rollback, follow `.agents/skills/_shared/skill-startup.md`, which reads `.template/project-profile.json`,
...
Read `.template/client-tools.local.json` first. Required values:
...
2. Run `ValidateTicketLock` when `.template/delivery-context.local.json` is present and report the active ticket lock. If
...
rollback target is a previously released commit. Example via the Gitea API (values from `.template/client-tools.local.json`
...
Add a ticket comment with marker:
