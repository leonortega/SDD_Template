...
`.template/project-profile.json` and the selected ticket adapter path; read provider-specific references only when that
adapter requires them.

## Shared Context

Before mutating ticket or repository state, follow `.agents/skills/_shared/skill-startup.md`, which reads
`.template/project-profile.json`, `.agents/skills/_shared/delivery-contract.md`, and
...
This skill owns initial creation of ignored `.template/delivery-context.local.json` for automatic delivery. OpenProject
...
(`.agents/skills/_shared/pipeline-workflow-telemetry.md`) with:
...
Read `.template/project-profile.json` first for the selected ticket provider, ticket key pattern, branch policy, and
adapter path. Read `.template/client-tools.local.json` only for selected adapter runtime
values. Fall back to `.template/client-tools.example.json` only for defaults and setup guidance, then apply
...
`.template/project-profile.json` or the selected repository adapter.
...
- Ignored `.template/tool-recommendations.local.json` is used only after project guidance discovery confirms local
...
Load credentials from `.template/client-tools.local.json` or optional environment overrides only. Avoid echoing request headers, tokens, or full credential-bearing URLs.
