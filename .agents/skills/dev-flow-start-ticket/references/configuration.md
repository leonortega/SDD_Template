...
Use `.template/client-tools.local.json` as the primary local configuration file. Keep `.template/client-tools.example.json`
...
section to `.template/client-tools.local.json`:
...
Keep the API key in ignored `.template/client-tools.local.json` only and never commit it. This configuration is used by
...
```bash
Copy-Item .\.template\client-tools.example.json .\.template\client-tools.local.json
```

Set `openProject.apiToken` in `.template/client-tools.local.json`. The token is sent as the `Authorization: Bearer` header.
