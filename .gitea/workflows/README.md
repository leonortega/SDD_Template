...
All CI workflows use pinned local job images built by `BuildGiteaActionsImages`. This command is mandatory before PR validation, deployment, or QA E2E runs. Local and CI validation must not rely on ad hoc host tool installs when a pinned Docker image is available.
