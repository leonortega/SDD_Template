...
infra/k8s/shared/database/    # one shared engine per env (StatefulSet + PVC + db.internal, NodePort per env via ports.json, ADR-0003)
...
infra/k8s/
├── shared/database/             # one engine per env: StatefulSet + PVC + db.internal, NodePort per env via ports.json (ADR-0003)
...
4. Build locally: `docker build -f apps/<appId>/Dockerfile apps/<appId>`
...
- **Shared database engine (ADR-0003)**: one StatefulSet/PVC service per environment namespace (`db.internal`,
  ClusterIP base upgraded to NodePort per env via the overlay service patches from `ports.json`). Apps connect via
  `db.internal` + their own logical database. The engine image is a project-wide choice
