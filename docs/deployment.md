...
- The Rancher Desktop lane sends live site/API Serilog events to local Seq through `RANCHER_APP_SEQ_URL`, default `http://host.docker.internal:5341`. It also captures sanitized Kubernetes pod logs after namespace deployment and posts them to Seq with environment, namespace, app, commit SHA, and image digest fields. The same capture writes `monitoring-summary.json` for Nexus evidence.
