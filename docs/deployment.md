...
The current profile uses Azure App Service for default deployed runtimes, Azure Monitor and Log Analytics for cloud-side observability, and local Grafana/Seq views for operator checks. The optional Rancher Desktop lane uses local Kubernetes namespaces as DEV, QA, and PROD runtime targets while keeping Nexus and Plane as the durable delivery record.
...
- The Rancher Desktop lane captures sanitized Kubernetes pod logs after namespace deployment and posts them to Seq with environment, namespace, app, commit SHA, and image digest fields. The same capture writes `monitoring-summary.json` for Nexus evidence.
...
The optional Rancher Desktop lane uses `.gitea/workflows/rancher-local-deploy.yml`. It builds `sddtemplate/site` and `sddtemplate/api` images, pushes them to a Nexus Docker hosted repository, writes `container-images.json`, deploys the digest-pinned images to `sdd-dev`, then promotes the exact same digest set to `sdd-qa` after DEV succeeds. Local PROD promotion is explicit and deploys an existing QA-approved commit's digest set to `sdd-prod`. The QA evidence path remains `qa/{ticketKey}/{runId}/qa-evidence.zip`; local QA branches use `qa-local/{ticketKey}` to avoid colliding with the Azure QA branch trigger.

Rancher Desktop observability evidence is attached to the artifact commit. DEV and QA deployment jobs verify site/API `/health`, capture sanitized pod logs, post those logs to Seq when `RANCHER_OBSERVABILITY_ENABLED=true`, and upload `monitoring-summary*.json` plus sanitized pod-log files to Nexus raw under `app/{commitSha}/`. QA additionally publishes `app/{commitSha}/qa-observability.json`, and the local E2E evidence bundle includes that file when available.
