...
QA automation starts with repo-native tests and PR validation. Browser-visible Blazor behavior should use Playwright MCP, the Browser plugin, or Playwright-style user-visible assertions when configured; one-off evidence belongs under ignored `artifacts/qa/**`, while reusable regression tests belong in the normal test tree through the implementation workflow. `playwright-interactive` is optional local guidance for persistent browser debugging, visual QA, viewport-fit checks, and deployed browser failure diagnosis; it does not replace committed Playwright tests or the official E2E QA gate. Implementation PRs must create committed automated coverage for every acceptance criterion before product-code handoff, including Playwright E2E tests when browser-level proof is required. The reusable deployed-QA browser/API regression suite is intentionally aimed at deployed QA apps, not local dev servers, and runs remotely from Gitea Actions through the selected provider QA trigger branch after QA deployment: Rancher Desktop uses `qa-local/{ticketKey}`. `quality-test-e2e` owns only E2E execution, evidence, and QA pass/fail classification; it does not create, repair, stage, or commit tests. QA Done requires acceptance criteria proven by executable assertions from existing committed tests against the deployed QA artifact. Smoke checks, screenshots, logs, and traces are supporting evidence only; the QA record must map criteria to committed assertions and cover relevant user workflow, API/backend effect, independent state, validation/boundary, error-handling, environment-correctness, and evidence-integrity scenarios. A deployed browser E2E failure must be reproduced and classified with Playwright MCP or the configured Browser/Playwright tool before app-code changes. Missing or defective committed tests are routed back to implementation/review, not fixed in QA. App code must remain product-only; never add JavaScript helpers, hidden hooks, test ids, bypasses, timing shims, or Playwright-specific behavior only to make E2E pass.
...
Repo-specific skill changes belong under `.codex/skills/**`. IDE-owned and global skill roots such as `C:\Users\mlortega\.codex\skills` are read-only for this repository except restore-only cleanup when a previous run changed those files by mistake.
...
The selected provider QA branch workflow runs the committed Playwright suite remotely against deployed QA URLs. Rancher Desktop uses `qa-local/{ticketKey}` with `RANCHER_QA_SITE_URL` and `RANCHER_QA_API_URL`. After QA deployment, the branch triggers remote evidence for existing tests and gives `quality-test-e2e` the normal place to rerun and prove committed Playwright E2E checks. If the committed suite cannot prove a ticket's acceptance criteria, QA records the missing coverage and leaves the ticket in QA as `FAIL` or blocked. Local runs are only for test authoring during implementation or diagnostics, not the official QA E2E gate. After a successful QA deploy, `app/{commitSha}/qa-targets.json` records the QA Site/API URLs in Nexus so local Playwright diagnosis can discover the deployed target; ignored `.codex/client-tools.local.json` may also store the current QA URLs. Use `npm run test:docker` in `tests/SDDTemplate.E2ETests` for local diagnostics so the pinned `agentic/e2e-ci:playwright-1.57.0-1` image supplies browsers and system dependencies without installing Chromium on the host. During QA, one-off exploratory probes and generated evidence belong under ignored `artifacts/qa/**` and must not be committed. After the remote evidence exists and the QA workflow records OpenProject Done plus the RC/release metadata, delete the remote `qa-local/{ticketKey}` branch; it is only a temporary trigger, not the durable audit record.
...
Feature work starts from a OpenProject work package and normally creates an OpenSpec proposal before implementation. Agents should read the OpenProject work package, active OpenSpec artifacts, the ticket context lock, relevant code, relevant tests, and quality-gate configuration before editing.
...
Implementation agents use `tdd` and `ponytail full` while adding or changing project code: map every acceptance criterion to committed automated coverage, write one public-interface behavior test, confirm RED, add the smallest working change, confirm GREEN, refactor only while green, prefer standard library and native framework features first, and avoid speculative abstractions. Implementation is complete only when OpenSpec tasks are complete, PR review feedback tasks are complete, touched behavior has targeted local validation or a documented reason for deferring to CI, reusable tests have been added or updated for every acceptance criterion, and a Gitea PR has review-agent coverage. Do not require agents to run the full CI-equivalent test/security suite locally before opening or updating a PR; Gitea PR validation is the authoritative full gate.

Ticket start now includes a readiness gate before branch, OpenProject status, ticket-lock, or OpenSpec mutation. Tickets are classified as `ready`, `refinable`, or `blocked`. Refinable tickets keep moving only after the managed OpenProject block records Scrum-ready planning details: problem or opportunity, user story, concrete acceptance criteria, scope or affected areas, dependencies or assumptions, validation expectations, risks, and definition of done. Blocked tickets stop before mutation when the product or technical intent is still too vague.
...
Feedback batches use OpenProject markers `IA generated PR feedback detected: {headSha}:{feedbackBatchId}` and `IA generated PR feedback fixes: {headSha}:{feedbackBatchId}`. The batch id is derived from sorted source ids, so late human comments on the same PR head are processed as a new batch instead of being skipped by an earlier fix. OpenProject remains `In Review` while late human feedback fixes are applied; ambiguous or conflicting human comments block handoff until clarified. Feedback-fix OpenProject comments are reviewer-facing summaries, not only automation evidence: after the stable marker, they must explain which reviewer comments were addressed, how IA resolved them, what changed, which validation passed, and what the reviewer should re-check before approving.
...
Every implementation must finish with a Context Findings Review. Durable findings update the matching file under `docs/` in the same PR. If there are no durable findings, the PR body and OpenProject handoff comment must state `Docs: no durable context changes`.
