...
QA automation starts with repo-native tests and PR validation. Browser-visible Blazor behavior should use the Browser plugin and Playwright-style user-visible assertions when configured; one-off evidence belongs under ignored `artifacts/qa/**`, while reusable regression tests belong in the normal test tree through an implementation workflow. The reusable deployed-QA browser/API regression suite lives under `tests/SDDTemplate.E2ETests`; it is intentionally aimed at deployed QA apps, not local dev servers, and normally runs remotely from Gitea Actions after QA deployment or from a ticket-specific `qa/{ticketKey}` branch.

Clean-code and architecture guidance should fit this repository's current size: keep changes small, preserve separation between delivery tooling and application code, avoid broad abstractions until they remove real complexity, and prefer observable behavior tests over implementation-detail tests. Blazor web UI changes should preserve accessible, responsive, user-visible behavior. ASP.NET Core REST/API endpoint changes, including minimal APIs such as `/health` and `/metrics`, should use clear route shape, validation, safe error responses, and integration tests. Security-sensitive changes should account for secrets, authorization, dependency risk, scanner evidence, and OWASP-aligned review.

When the recommendation audit finds missing skills or guidance for detected tools, frameworks, environments, test frameworks, code standards, security, QA, or architecture, agents must use `project-guidance-discover` first. `project-guidance-discover` builds a `project-guidance-search-plan` from detected signals, researches official sources first, falls back to technology-owner or widely used public sources only when no official source exists, shows suggested missing skills and guidance, and asks whether the user wants to add additional desired items. Only after that confirmation may `project-guidance-acquire` copy the final confirmed skill items into `.codex/skills/`; non-skill guidance stays in `.codex/tool-recommendations.local.json`. Use `project-guidance-mapper` to decide which repo-local workflow skills, installed expert skills, tools, references, practices, and standards apply to config, ticket start, planning, implementation, review, QA, deploy, rollback, hotfix, and retrospective steps.
