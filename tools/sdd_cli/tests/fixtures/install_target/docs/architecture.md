...
Installed repos get only runtime workflow assets: Codex skills, docs, OpenSpec config, infrastructure templates, workflow templates, common non-secret config, and `tools/sdd_cli`. Tool-source-only files such as CLI tests, repo memory, OpenSpec changes, eval output, telemetry, and local config are not installed.
