...
def run_eval(root: Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise CliError("promptfoo is not available. Install with: npm install -g promptfoo")
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--no-cache",
                "--output", str(base / ".codex" / "agent-evals" / "results.tmp.json"),
            ],
            capture_output=True,
            text=True,
            timeout=300,
        ...
        raise CliError("Eval timed out after 300 seconds.")
...
def show_view(root: Path | None = None) -> int:
    ...
    base = root or REPO_ROOT
    try:
        subprocess.run(["npx", "promptfoo", "view"], cwd=base, check=False)
        return 0
