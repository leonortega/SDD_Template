...
def build_monorepo_index(repo_path, project_filter=None):
    ...
    if (
        ...
        and (current_time - _INDEX_CACHE["last_check_time"]) < 5
    ):
    ...
    if (
        _INDEX_CACHE["last_project"] == cache_key
        and _INDEX_CACHE["max_mtime"] == current_max_mtime
        and _INDEX_CACHE["retriever"] is not None
    ):
    ...
    _INDEX_CACHE["indexed_data"] = list(zip(file_paths, documents, strict=False))
...
def search_documentation(
    query: str, subproject: str | None = None, force_refresh: bool = False
) -> str:
    """
        ...
        if not isinstance(idx, int):
            try:
                ...
                try:
                    ...
                    raise TypeError(
                        f"Unable to convert search result index {raw_idx!r} to int"
                    ) from exc
