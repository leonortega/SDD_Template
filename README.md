...
The template ships no stack-specific implementation files. After you define the tech stack (`configure set-project-stack`), the setup process creates the deterministic skeleton `src/` and `tests/`, then the AI-driven `dev-flow-scaffold-project` skill resolves every stack-specific artifact (e.g., `package.json` + `playwright.config.ts` for JS/TS web stacks, native tooling for others). No fixed template list — the AI decides from the selected stack.
