...
- [ ] 1.1 Confirm whether implementation should proceed as a chained PR plan or as a size exception for one PR.
- [ ] 1.2 Inspect current Client data/API/UI/tests and identify reusable patterns without introducing generic CRUD abstractions.
- [ ] 1.3 Confirm current authentication and authorization capabilities before implementing Products access restrictions.

## 2. Data And API

- [ ] 2.1 Add Product entity, DbContext mapping, and migration for Name, SKU, Status, Price, Category, and Last Updated.
- [ ] 2.2 Add Product DTOs and CRUD endpoints for list, read, create, update, and delete.
- [ ] 2.3 Add Product validation for required fields, invalid SKU/status/price values, missing records, and duplicate SKU if supported.
...
- [ ] 4.1 Add migration/schema tests for Product storage.
- [ ] 4.2 Add API tests for Product list, read, create, update, delete, validation, not-found, and unauthorized behavior.
...
- [ ] 5.1 Run focused build/test validation while implementing, then run configured quality gates before PR handoff.
- [ ] 5.2 Run ponytail full mode simplification review on the current diff and apply actionable recommendations.
